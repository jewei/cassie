                            ...
                            <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                                ...
                                <table class="min-w-full divide-y divide-gray-300">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" class="px-3 py-3.5 text-sm font-semibold text-gray-900">Participants</th>
                                    ...
                                    <tr v-for="(template, idx) in templates" :key="template.id" :class="idx % 2 === 0 ? undefined : 'bg-gray-50'">
                                        <td class="whitespace-nowrap px-3 py-4 text-right text-gray-500"><img :src="template.thumbnail_url" class="h-16 w-24 object-cover" /></td>
                                        <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{ template.name }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500">{{ template.certificates_count }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-left text-sm text-gray-500">
                                            <Link :href="route('templates.certificates.index', template)" class="text-indigo-600 hover:text-indigo-900" preserve-scroll>
                                                {{ template.certificates_count > 0 ? 'Edit participants' : 'Add participants' }}
                                            </Link>
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ template.updated_at }}</td>
                                        <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                            <Link :href="route('templates.edit', template)" class="text-indigo-600 hover:text-indigo-900" preserve-scroll>Edit</Link>
                                        </td>
                                    </tr>
                                    </tbody>
                                </table>
